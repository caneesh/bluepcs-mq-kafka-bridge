...
@DisplayName("HdfsConnectionCheck HA nameservice probing")
class HdfsConnectionCheckTest {

    @SuppressWarnings("unchecked")
    private HdfsConnectionCheck checkWith(Configuration conf) {
        ObjectProvider<Configuration> provider = mock(ObjectProvider.class);
        when(provider.getIfAvailable()).thenReturn(conf);
        return new HdfsConnectionCheck(provider);
    }
    ...
    void passesWhenOneNamenodeReachable() throws Exception {
        try (ServerSocket listening = new ServerSocket(0)) {
            ...
            CheckResult result =
                    checkWith(conf).checkHaNameservice("HDFS_CONNECTION", "PRDODPHA");

            assertThat(result.getStatus()).isEqualTo(CheckResult.Status.PASS);
    ...
    void failsWithoutHaMapping() {
        ...
        assertThat(result.getStatus()).isEqualTo(CheckResult.Status.FAIL);
    ...
    void failsWithoutHadoopConfiguration() {
        CheckResult result =
                checkWith(null).checkHaNameservice("HDFS_CONNECTION", "PRDODPHA");
